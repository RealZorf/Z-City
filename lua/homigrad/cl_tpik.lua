...
function hg.DoTPIK(ply, ent)
    ...
    if lerp_rh != 0 then
        ...
        q = q * Quaternion():SetAngleAxis(angrr.y, Vector(0, 0, 1))
        q = q * Quaternion():SetAngleAxis(angrr.p, Vector(0, 1, 0))
        q = q * Quaternion():SetAngleAxis(-180 - 90 + eyeang.r - angrr.r - math.NormalizeAngle((eyeang.y - angrr.y)) * (math.NormalizeAngle(angrr.p)) / 90, Vector(1, 0, 0))
        --q:SetAngleAxis(-angle2 + 180, Vector(0, 1, 0))
        --q:SetAngleAxis(180, Vector(1, 0, 0))
        local ang = q:Angle()
        ...
        local wmat = wrst and ent:GetBoneMatrix(wrst)
        if wrst and wmat then
            ang:RotateAroundAxis(ang:Forward(), angrotate * 0.5 - 90 + 30)
            wmat:SetAngles(ang)
            ent:SetBoneMatrix(wrst, wmat)
        end
        ...
        local wmat = wrst and ent:GetBoneMatrix(wrst)
        if wrst and wmat then
            ang:RotateAroundAxis(ang:Forward(), angrotate * 0.5 + 90)
            wmat:SetAngles(ang)
            ent:SetBoneMatrix(wrst, wmat)
        end
    end
    ...
    if lerp_lh != 0 then
        ...
        local wmat = wrst and ent:GetBoneMatrix(wrst)
        if wrst and wmat then
            ang:RotateAroundAxis(ang:Forward(), angrotate * 0.5 - 180)
            wmat:SetAngles(ang)
            ent:SetBoneMatrix(wrst, wmat)
        end
        ...
        local wmat = wrst and ent:GetBoneMatrix(wrst)
        if wrst and wmat then
            ang:RotateAroundAxis(ang:Forward(), angrotate * 0.5 + 60)
            wmat:SetAngles(ang)
            ent:SetBoneMatrix(wrst, wmat)
        end
